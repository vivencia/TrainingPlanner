...
public:	
	...
	Q_INVOKABLE inline bool isSelected(const uint index) const { return m_modeldata.at(m_indexProxy.at(index)).at(EXERCISES_COL_SELECTED) == u"1"_qs; }
	Q_INVOKABLE inline void setSelected(const uint index, const bool selected)
	{
		m_modeldata[m_indexProxy.at(index)][EXERCISES_COL_SELECTED] = selected ? u"1"_qs : u"0"_qs;
		addModifiedIndex(index);
	}

...
void DBTrainingDayTable::updateFromModel()
{
	m_result = false;
	if (mSqlLiteDB.open())
	{
		...
		static_cast<DBTrainingDayModel*>(m_model)->updateFromModel(model);
	...
	if (!m_result)
	{
		MSG_OUT("DBTrainingDayTable updateFromModel Database error:  " << mSqlLiteDB.lastError().databaseText())
		MSG_OUT("DBTrainingDayTable updateFromModel Driver error:  " << mSqlLiteDB.lastError().driverText())
	}
	else
	{
		m_model->clearModifiedIndices();
		MSG_OUT("DBTrainingDayTable updateFromModel SUCCESS")
	}
	doneFunc(static_cast<TPDatabaseTable*>(this));
}

...
TPSettings::TPSettings(QObject* parent) : QSettings{parent}
{
	...
	m_propertyNames << std::move(u"appVersion"_s) << std::move(u"appLocale"_s) << std::move(u"widgetTheme"_s) << std::move(u"colorScheme"_s) <<
		...
		std::move(u"lastViewedMesoIdx"_s) << std::move(u"pageWidth"_s) << std::move(u"pageHeight"_s) << std::move(u"itemMaxWidth"_s) <<
		std::move(u"alwaysAskConfirmation"_s) << std::move(u"mainUserConfigured"_s) << std::move(u"weatherCities"_s);
	...
	m_defaultValues << std::move(TP_APP_VERSION) << std::move(u""_s) << std::move(u"Material"_s) << std::move(u"Blue"_s) <<
		...
		FONT_POINT_SIZE_TITLE << STR_MINUS_ONE << std::move(u"360"_s) << std::move(u"600"_s) << std::move(u"340"_s) <<
		STR_ONE << STR_ZERO << std::move(u"São Paulo"_s);

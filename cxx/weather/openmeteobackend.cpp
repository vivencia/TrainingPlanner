...
*/
static QString weatherCodeToIcon(const int code)
{
	switch (code)
	{
		case 0: return "sunny"_L1;
		case 1: return "sunny-very-few-clouds"_L1;
		case 2: return "few-clouds"_L1;
		case 3: return "overcast"_L1;
		case 45:
		case 48: return "fog"_L1;
		case 51:
		case 53:
		case 55:
		case 56:
		case 57: return "showers-scattered"_L1;
		case 61:
		case 63:
		case 65:
		case 80:
		case 81:
		case 82: return "showers"_L1;
		case 66:
		case 67: return "sleet"_L1;
		case 71:
		case 73:
		case 75:
		case 77:
		case 85:
		case 86: return "snow"_L1;
		case 95:
		case 96:
		case 99: return "thundershower"_L1;
		default: return "sunny"_L1;
	}
}
...
/*
	...
	https://open-meteo.com/en/docs
*/
static QString weatherCodeToDescription(const int code)
{
	switch (code)
	{
		case 0: return "clear sky"_L1;
		case 1: return "mainly clear"_L1;
		case 2: return "partly cloudy"_L1;
		case 3: return "overcast"_L1;
		case 45:
		case 48: return "fog"_L1;
		case 51:
		...
		case 55:
		case 56:
		case 57: return "drizzle"_L1;
		case 61:
		case 63:
		case 65: return "rain"_L1;
		case 66:
		case 67: return "freezing rain"_L1;
		case 71:
		case 73:
		case 75:
		case 77:
		...
		case 80:
		case 81:
		case 82: return "pouring rain"_L1;
		case 95:
		case 96:
		case 99: return "thunderstorm"_L1;
		default: return QString();
	}
}

// The supported cities are taken from WeatherInfo.qml. Need to keep in sync with it.
...
OpenMeteoBackend::OpenMeteoBackend(QObject* parent)
	: ProviderBackend{parent}, m_networkManager{new QNetworkAccessManager(this)}
{}

void OpenMeteoBackend::requestWeatherInfo(const QString& city)
{
	const QGeoCoordinate& coordinate{coordinateForCity(city)};
...
void OpenMeteoBackend::handleWeatherForecastReply(QNetworkReply* reply, const st_LocationInfo& location)
{
	if (!reply)
	{
		emit errorOccurred();
	...
	QList<st_WeatherInfo> weatherData;
	if (!reply->error())
	{
		...
		if (days.size() >= kMaxEntries && weatherCodes.size() >= kMaxEntries
					&& maxTemperatures.size() >= kMaxEntries && minTemperatures.size() >= kMaxEntries)
		{
			for (qsizetype i(0); i < kMaxEntries; ++i)
			{
				...
				if (unixTime > 0)
				{
					...
					info.m_dayOfWeek = std::move(date.toString("ddd"_L1));
				}
				...
				info.m_temperature = std::move(niceTemperatureString(minTemp) + u'/' + niceTemperatureString(maxTemp));

				if (!info.m_dayOfWeek.isEmpty() && !info.m_temperature.isEmpty() && !info.m_weatherIconId.isEmpty())
...
void OpenMeteoBackend::generateWeatherRequest(const QString& city, const QGeoCoordinate& coordinate)
{

...
public:
	...
	inline bool mainUserConfigured() const { return value(m_propertyNames.at(USER_INDEX), m_defaultValues.at(USER_INDEX).toUInt()).toBool(); }
	inline void setMainUserConfigured(const bool new_value) { changeValue(USER_INDEX, QString::number(new_value)); emit mainUserConfiguredChanged(); }
